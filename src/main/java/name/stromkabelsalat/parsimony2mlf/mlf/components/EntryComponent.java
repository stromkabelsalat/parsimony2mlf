...
public class EntryComponent {
	...
	public Entry entry(final ParsimonyMessage parsimonyMessage) {
		final Integer id = this.mlfValuesComponent.id(parsimonyMessage.getEpochNum(), parsimonyMessage.getBoardNum(), parsimonyMessage.getNum());

		log.info("Creating Entry {}", id);

		final Integer pid = Optional
				.ofNullable(parsimonyMessage.getParentNum())
				.map(parentNum -> this.mlfValuesComponent.id(parsimonyMessage.getEpochNum(), parsimonyMessage.getBoardNum(), parentNum))
				.orElse(0);

		final Integer tid = Optional
				.of(pid)
				.filter(pidCandidate -> pidCandidate != 0)
				.map(pidCandidate -> this
						.idTids
						.stream()
		...
		final LocalDateTime time = parsimonyMessage.getDate();
		...
		final Integer category = this.mlfValuesComponent.category(parsimonyMessage.getEpochNum(), parsimonyMessage.getBoardNum());
		...
		.ifPresent(img -> textBuilder
				.append("[img]themes/default/images/image.png[/img] [img]" + img + "[/img]")
		...
		.ifPresent(url -> textBuilder
				.append("[link=" + url + "][img]themes/default/images/homepage.png[/img][/link] [b][link=" + url + "]" + parsimonyMessage.getUrlTitle() + "[/link][/b]"));

		final String textWithOriginalLinks = textBuilder.toString().strip();
		final String text = this.linkReplacerComponent.replaceLinks(textWithOriginalLinks, parsimonyMessage.getEpochNum(), parsimonyMessage.getBoardNum());

		final String editKey = "";
		final String hpWithOriginalLink = Optional.ofNullable(parsimonyMessage.getUrl()).orElse("");
		final String hp = this.linkReplacerComponent.replaceLinks(hpWithOriginalLink, parsimonyMessage.getEpochNum(), parsimonyMessage.getBoardNum());

		final String location = Optional
				.ofNullable(parsimonyMessage.getNameColor())
				.map(nameColor -> "Stammposter")
				.orElse("");
