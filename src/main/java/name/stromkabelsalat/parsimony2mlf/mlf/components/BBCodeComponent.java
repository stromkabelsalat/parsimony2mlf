...
import name.stromkabelsalat.parsimony2mlf.mlf.colors.repositories.ValidColorHexRepository;

@Component
public class BBCodeComponent {
	private final ValidColorHexRepository validColorHexRepository;
	...
	public BBCodeComponent(final ValidColorHexRepository validColorHexRepository,  final UniqueStringComponent uniqueStringComponent) {
		this.validColorHexRepository = validColorHexRepository;
	...
	public String bbCode(final String body) {
		...
		.forEach(element -> element
				.before("[color=" + this.validColorHexRepository.findMostSimilarTo(element.attr("color")) + "]")
				.after("[/color]"));
