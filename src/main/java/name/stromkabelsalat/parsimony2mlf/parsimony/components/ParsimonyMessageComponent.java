...
public class ParsimonyMessageComponent {
	...
	public ParsimonyMessage parsimonyMessage(final MessageDataRecord messageDataRecord) {
		...
		ParsimonyMessageComponent.log.info("Creating ParsimonyMessage {} {} {}", epochNum, boardNum, num);
		...
		Optional.of(sections[2]).ifPresent(section -> {
			...
			final InnerOuter antwortInnerOuter = this.innerOuterComponent.innerOuter(rawBody, InnerOuterComponent.ANTWORT_REGEX);
			...
			final Integer parentNum = Optional
					...
					.map(Integer::valueOf)
					.filter(parentNumCandidate -> this.messageDataRecordRepository.existsByEpochNumAndBoardNumAndNum(epochNum, boardNum, parentNumCandidate))
					.orElse(this
							.parsimonyNodeRepository
			...
			final InnerOuter geschriebenInnerOuter = this.innerOuterComponent.innerOuter(rawBody, InnerOuterComponent.GESCHRIEBEN_REGEX);
			...
			final InnerOuter imgInnerOuter = this.innerOuterComponent.innerOuter(rawBody, InnerOuterComponent.IMG_REGEX);
			...
			final InnerOuter urlInnerOuter = this.innerOuterComponent.innerOuter(rawBody, InnerOuterComponent.URL_REGEX);
