...
public class QuirksReplacerComponent {
	...
	public QuirksReplacerComponent(@Value("${name.stromkabelsalat.parsimony2mlf.paths.quirks-replacements}") final Path quirksReplacementsPath, final ObjectMapper objectMapper, final SeparatorComponent separatorComponent) throws IOException {
		this.separator = separatorComponent.getSeparator();

		final InputStream inputStream = Files.newInputStream(quirksReplacementsPath);

		final Object object = objectMapper.readerForMapOf(String.class).readValue(inputStream);
	...
	public String replaceQuirks(final String string) {
		...
		result = result
				...
				.replaceAll(IGNORE_CASE_REGEX + Pattern.quote("</center>" + System.lineSeparator() + "<hr>" + System.lineSeparator() + "<center>"), "</center> " + this.separator + "<center>")
				.replaceAll(IGNORE_CASE_REGEX + Pattern.quote("</center>" + System.lineSeparator() + "<hr>"), "</center> " + this.separator)
				.replaceAll(IGNORE_CASE_REGEX + Pattern.quote("<hr></body>"), this.separator)
				.replaceAll(IGNORE_CASE_REGEX + Pattern.quote("<hr>" + System.lineSeparator() + "</body>"), this.separator)
				.replaceAll(IGNORE_CASE_REGEX + Pattern.quote("<hr>" + System.lineSeparator() + System.lineSeparator() + "</body>"), this.separator);
